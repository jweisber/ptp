...
Suppose we try to calculate the expected utility of believing in Catholicism, for example. We're going to have $Pr(\mbox{Catholicism}) \times \infty$ in the mix, which is equal to $\infty$ as we saw earlier. But we're also going to have $Pr(\mbox{Bashful}) \times -\infty$ in there, which equals $-\infty$. So at some point we're going to have to deal with $\infty - \infty$.
